...
#include <arkod/arkstep.h>             /* access to ARKode func., consts. */
...
int ARKStep_TemperatureSolve( realtype t, N_Vector u, N_Vector uDot, void* voidPlasma )
{
	...
	// plasma->UpdateVoltage(t) <-- if supplied a time-dependent voltage	
...
void MCTransConfig::doTempSolve( MirrorPlasma& plasma ) const
{
	...
	const unsigned long MaxSteps = 1e6;
	ArkodeErrorWrapper( ARKStepSetMaxNumSteps( arkMem, 1e6 ), "ARKStepSetMaxNumSteps" );
